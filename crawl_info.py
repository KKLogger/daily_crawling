...
def get_car_info(url, temp):
    ...
    carHistorySeq = soup_t[
        soup_t.index('carHistorySeq = "')
        + str_len : soup_t.index('"', soup_t.index('carHistorySeq = "') + str_len)
    ]
    chk_tag_url = soup.find("li", {"class": "used01"}).find("a")["data-link-url"]
    ...
    json_data = {"gotoPage": "1", "pageSize": "1", "carSeqVal": carSeq}
    try:
        time.sleep(random.uniform(0.1, 0.3))
    ...
    try:
        if soup.find("div", {"class": "suc-price"}) is None:
            if (
                soup.find_all("div", {"class": "car-buy-debt-m"})[1].find("div").text
                == "리스 이용 금융상담문의"
            ):
    ...
    if (
        soup.find("div", {"class": "dealer-info-area"}).find("span", {"place-add"}).text
        == "개인판매자"
    ):
    ...
    extend_service = soup.find("div", {"class": "service-list"}).find("a").get("class")
    ...
    dealer_location = (
        dealer_info.find("span", {"class", "place-add"}).text.strip().split(" ")[0]
    )
    dealer_phone = dealer_info.find("div", {"class": "dealer-tel-num"}).text.strip()
    ...
    if full_txt is None:
        ...
        dealerNo = full_txt[
            full_txt.index(find_txt) + find_txt_len : full_txt.index(")")
        ].split(",")
...
def get_checkdata(url, temp, chk_tag_url):
    ...
    if "http" in chk_tag_url:  # 다른 페이지로 이동
        ...
        carSeq = url[url.index("?carSeq=") + len("?carSeq=") :]
        data = {"layerId": "layerCarCheckInfo", "carSeq": carSeq}
        time.sleep(random.uniform(0.1, 0.3))
        res = requests.post(
...
def get_options(url, option_codes):
    ...
    """
    time.sleep(random.uniform(0.1, 0.3))
    res = requests.post("https://www.kbchachacha.com/public/car/option/code/list.json")
    ...
    columns = [
        ...
        "제논라이트(HID)",
        "aluminium_wheel",
        "electrically_folded_side_mirror",
        "주간주행등(DLR)",
        "파노라마썬루프",
        "loop_rack",
        "LED리어램프",
        ...
        "head_lamp",
        "heated_sheet_(front)",
        "ventilation_sheet_(left)",
        "power_steering_wheel",
        "steering_wheel_remote_control",
        "leather_seat",
        "electric_sheet_(left)",
        "electric_sheet_(right)",
        "electric_seat_(back_seat)",
        "memory_sheets_(left)",
        "massage_sheet",
        "슈퍼비전계기판",
        "paddle_shift",
        "high_pass",
        "memory_sheets_(right)",
        "heated_sheet_(rear)",
        "엠비언트라이트",
        "워크인시트",
        "전동햇빛가리개",
        "ventilation_sheet_(right)",
        "ventilation_seat_(back_seat)",
        "rear_side_alert_system",
        "사이드&커튼에어백",
        "airbag_(left)",
        "airbag_(right)",
        "rear_camera",
        "360_degree_around_view",
        "블랙박스",
        "anti_lock_brake_(ABS)",
        "electronic_control_suspension_(ECS)",
        "anti_slipping_(TCS)",
        "body_position_control_(ESC)",
        "lane_departure_alarm_system_(LDWS)",
        "도난경보기",
        "tire_pressure_sensor_(TPMS)",
        "무릎에어백",
        "parking_detection_sensor",
        "전방카메라",
        "자동긴급제동(AEB)",
        "smart_key",
        "power_window",
        ...
        "CD_player",
        "CD체인저",
        "front_seat_AV_monitor",
        "MP3",
        "AUX_terminal",
        "USB_terminal",
        "핸즈프리",
        "iPod 단자",
        "rear_seat_AV_monitor",
        "스마트폰미러링",
        "내비게이션 (비순정)",
        "bluetooth",
    ]
...
def get_optioncodes(url):
    ...
    carSeq = url[index:]
    data = {"layerId": "layerCarOptionView", "carSeq": carSeq}
    res = requests.post(
        "https://www.kbchachacha.com/public/layer/car/option/list.kbc",
        headers=headers,
        data=data,
    )
    soup = bs(res.text, "html.parser")
...
def get_history(url, temp, carHistorySeq):
    ...
    datas = {"layerId": "layerCarHistoryInfo", "carHistorySeq": carHistorySeq}
    time.sleep(random.uniform(0.1, 0.3))
    response = requests.post(
        "https://www.kbchachacha.com/public/layer/car/history/info.kbc",
        headers=headers,
        data=datas,
    )
    soup = bs(response.text, "html.parser")
    ...
    historys = soup.find_all("div", {"class": "cmm-table table-l02 ct-line td-ptb-15"})
    HistDamage = dict()
    ...
    try:
        hide_list = soup.find("ul", {"class": "hide-list"}).find_all(
            "span", {"class": "txt"}
        )
...
def crawl_iframe(url, temp, soup):
    ...
    if temp["MotorType"] == "":
        temp["MotorType"] = "null"
    temp["WarrantyType"] = table[0].find_all("tr")[9].find("div").get("value")
    ...
    check_inner["BrakeMasterCylinderOilLeakage"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[29]
        .find("div", {"class": "option-ch"})
        .get("value")
    )
    check_inner["BrakeOilLeakage"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[30]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["ElectricGeneratorOutput"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[32]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["ElectricIndoorBlowerMotor"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[35]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["ElectricRadiatorFanMotor"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[36]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["ElectricStarterMotor"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[33]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["ElectricWindowMotor"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[37]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["ElectricWiperMotorFunction"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[34]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["SelfCheckTransmission"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[1]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorOperationStatus"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[2]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorOilLeakLockerArmCover"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[3]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorOilLeakCylinderHeaderGasket"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[4]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorOilLeakOilFan"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[5]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorWaterLeakCylinderHeaderGasket"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[7]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorWaterLeakPump"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[8]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorWaterLeakRadiato"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[9]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorWaterLeakCoolingRate"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[10]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["MotorHighPressurePump"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[11]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["TransAutoOilLeakage"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[12]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["TransAutoOilFlowAndCondition"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[13]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["TransManualOilLeakage"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[15]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["TransManualGearShifting"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[16]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["TransManualFluidFlowAndCondition"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[17]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["PowerClutchAssembly"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[19]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["PowerConstantVelocityJoint"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[20]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["PowerWeightedShaftAndBearing"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[21]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["SteeringPowerOilLeakage"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[23]
        .find("div", {"class": "option-ch"})
    ...
    check_inner["SteeringTieRodEndAndBallJoint"] = (
        table[4]
        .find("tbody")
        .find_all("tr")[28]
        .find("div", {"class": "option-ch"})
    ...
    for i, item in enumerate(items_index):
        ...
        if (
            ...
            item == " "
            and check_inner["MotorOperationStatus"] != "null"
            and check_inner["SelfCheckMotor"] != "null"
        ):
